...
from fatcat_agent.conversations import ConversationStore

LOG = logging.getLogger("fatcat_agent")
...
class FatCatAgentSession:
    ...
    def _make_agent(self):
        ...
        return AIAgent(
            model=os.environ.get("FATCAT_MODEL", ""),
            platform="fatcat",
            quiet_mode=True,
...
class _FatCatACPBridge:
    ...
    def __init__(self, server: "FatCatAgentServer"):
        self.server = server
...
class FatCatAgentServer:
    def __init__(self, socket_path: Path, hermes_home: Path, config_bridge=None, conversation_store=None, allow_shutdown: bool | None = None):
        ...
        self.sessions: dict[str, FatCatAgentSession] = {}
    ...
    async def handle_message(self, message: dict[str, Any], emit) -> dict[str, Any] | None:
        ...
        if message_type == "new_session":
            ...
            self.sessions[state.session_id] = FatCatAgentSession(
        ...
        if message_type == "load_session":
            ...
            self.sessions[session_id] = FatCatAgentSession(
...
def main() -> None:
    ...
    asyncio.run(FatCatAgentServer(args.socket, args.hermes_home).start())
