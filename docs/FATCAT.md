...
FatCat is a portable Bible Strong avatar definition. This guide shows how to put the same little blue pet into other React projects: assistants, empty states, onboarding, buttons, loading screens, docs, and prototypes.

The source of truth is [`public/fatcat.avatar.json`](../public/fatcat.avatar.json). The filename is kept for compatibility with the original lab export; the definition itself is named `FatCat`.

## What travels with FatCat
...
Copy the JSON into a source folder, for example `src/avatars/fatcat.avatar.json`, then create a reusable component:

```tsx
import { createAvatar } from '@bible-strong/avatar-react'
import fatcatDefinition from './avatars/fatcat.avatar.json'

const FatCatAvatar = createAvatar(fatcatDefinition)

export function FatCatIdle() {
  return (
    <FatCatAvatar
      defaultAnimation="idle"
      size={160}
      ariaLabel="FatCat, your study companion"
    />
  )
}
```
...
Use a direct expression when FatCat is reacting to a specific UI state:

```tsx
<FatCatAvatar
  expression="curious-left"
  size={96}
  ariaLabel="FatCat looks curious"
/>
```
...
Use an animation when FatCat should feel alive over time:

```tsx
<FatCatAvatar
  defaultAnimation="thinking"
  size="min(42vw, 280px)"
  ariaLabel="FatCat is thinking"
/>
```
...
type FatCatProps = {
  ...
  size?: ComponentProps<typeof FatCatAvatar>['size']
  className?: string
  ariaLabel?: string
}
...
```tsx
<FatCat state="idle" size={72} />
<FatCat state="listening" size={120} />
<FatCat state="thinking" size={180} />
<FatCat state="happy" size={64} />
```

Keep the wrapper semantic. Product copy should explain what FatCat is doing; the avatar should support the moment, not become the entire message.
...
<section className="empty-state">
  <FatCat state="curious" size={112} ariaLabel="FatCat is curious" />
  <h2>No practice set yet</h2>
  <p>Pick a subject and FatCat will help you find a useful next question.</p>
</section>
```
...
<div className="loading-state" role="status" aria-live="polite">
  <FatCat state="working" size={88} ariaLabel="FatCat is working" />
...
<article className="welcome-card">
  <FatCat state="happy" size={144} ariaLabel="FatCat welcomes you" />
  <div>
...
<FatCatAvatar
  ...
  size={32}
  ariaLabel="FatCat is listening"
/>
```

## Buttons and programmatic reactions

For a component that changes FatCat in response to events, use `Avatar` with an `AvatarController` ref:
...
export function FatCatControls() {
  ...
  return (
    <div>
      <Avatar
        ref={avatar}
        definition={fatcatDefinition}
        defaultAnimation="idle"
        size={180}
        ariaLabel="FatCat study companion"
      />
...
export function FatCat() {
  return <FatCatAvatar defaultAnimation="idle" size={144} ariaLabel="FatCat" />
}
```
...
```tsx
<FatCatAvatar size={64} />
<FatCatAvatar size="clamp(96px, 20vw, 240px)" />
```
...
.fatcat-slot .bs-avatar {
  filter: drop-shadow(0 14px 18px rgb(44 63 105 / 12%));
}
```

Use one calm placement per surface. FatCat works best as a clear focal object or a small status companion; avoid stacking multiple decorative glows around it.
...
- Keep buttons that control FatCat as native `<button>` elements with visible focus states.

## License note

The installed `@bible-strong/avatar-react` package declares `AGPL-3.0-only`. Before embedding FatCat into a distributed or hosted product, review the package license and your project’s obligations with the person responsible for legal/compliance. Keep the package license and attribution with copied assets.
...
The lab is intentionally the catalog: it shows the actual 28 expressions and 23 animation timelines available in FatCat’s definition, without inventing aliases that are not in the JSON.
