# Peppa Anywhere Desktop Pet Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the dashboard prototype with a packaged, transparent native Peppa desktop pet whose attached chat uses the installed Hermes Agent runtime.

**Architecture:** AppKit owns a borderless transparent `NSPanel`, status item, native SwiftUI bubble, and menus. A transparent WKWebView renders only the real Peppa avatar definition; a native ACP stdio client launches `hermes acp` and preserves one session. ScreenCaptureKit remains native and emits privacy-filtered structured metadata.
...
- Test: `macos/PeppaAnywhere/Tests/PeppaAnywhereCoreTests/NativeDomainTests.swift`
...
- [ ] Run `swift test --package-path macos/PeppaAnywhere --filter NativeDomainTests` and confirm failure is caused by missing contracts.
- [ ] Write failing Vitest checks that the production entrypoint renders `PeppaAvatar` only, references transparent root styling, and contains no `CompanionDashboard` import.
...
**Files:**
- Create: `macos/PeppaAnywhere/Sources/PeppaAnywhereCore/HermesACP.swift`
- Modify: `macos/PeppaAnywhere/Sources/PeppaAnywhereCore/NativeDomain.swift`
- Test: `macos/PeppaAnywhere/Tests/PeppaAnywhereCoreTests/HermesACPTests.swift`
...
**Files:**
- Modify: `src/App.tsx`
- Modify: `src/components/PeppaCompanionAvatar.tsx`
...
- [ ] Keep all avatar definition data sourced from `public/strobi.avatar.json` and remove lab/dashboard-only layout from the production path.
...
- [ ] Wire pause/resume to `CaptureCoordinator` and expose status visibly in both Peppa and the status item.
...
- Create or modify: `scripts/smoke-peppa-macos.sh`
...
- [ ] Update packaging to build `Peppa Anywhere.app`, copy resources, set `CFBundleIdentifier=com.buyan.peppa-anywhere`, preserve Screen Recording usage text, and sign consistently when possible.
...
- [ ] Run `npm test`, `npm run lint`, `npm run build`, `swift test --package-path macos/PeppaAnywhere`, `swift build --package-path macos/PeppaAnywhere`, packaging, verification, and the smoke script.
