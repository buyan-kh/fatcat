...
public enum FatCatAgentState: String, Codable, Equatable, Sendable {
    case connecting
    case ready
    case sending
    case idle
...
public enum FatCatIPCMessage: Equatable, Sendable {
    case hello
    ...
    case state(FatCatAgentState)
    case sessionState(FatCatAgentState, sessionID: String, requestID: String?)
...
public enum FatCatIPCError: Error, Equatable, Sendable {
...
public enum FatCatIPCCodec {
    public static func encode(message: FatCatIPCMessage) throws -> Data {
        let object = try object(for: message)
        let data = try JSONSerialization.data(withJSONObject: object, options: [.sortedKeys])
        return data + Data([0x0A])
    }

    public static func decodeLine(_ line: String) throws -> FatCatIPCMessage {
        try decodeLine(Data(line.utf8))
    }

    public static func decodeLine(_ line: Data) throws -> FatCatIPCMessage {
        let trimmed = line.drop { $0 == 0x0A || $0 == 0x0D || $0 == 0x20 || $0 == 0x09 }
        guard !trimmed.isEmpty else { throw FatCatIPCError.malformed("empty line") }
        let object: Any
        do {
            object = try JSONSerialization.jsonObject(with: Data(trimmed))
        } catch {
            throw FatCatIPCError.malformed("invalid JSON")
        }
        guard let dictionary = object as? [String: Any] else { throw FatCatIPCError.malformed("message must be an object") }
        try rejectCredentials(in: dictionary)
        guard let version = dictionary["version"] as? Int else { throw FatCatIPCError.malformed("missing version") }
        if version == 2 {
            guard let kind = dictionary["kind"] as? String,
                ...
                throw FatCatIPCError.malformed("invalid Hermes event")
            }
        ...
        guard version == 1 else { throw FatCatIPCError.unsupportedVersion(version) }
        guard let type = dictionary["type"] as? String else { throw FatCatIPCError.malformed("missing type") }
        func string(_ key: String) throws -> String {
            guard let value = dictionary[key] as? String else { throw FatCatIPCError.malformed("missing \(key)") }
            return value
        }
        func bool(_ key: String) throws -> Bool {
            guard let value = dictionary[key] as? Bool else { throw FatCatIPCError.malformed("missing \(key)") }
            return value
        }
        ...
        case "conversation_snapshot":
            guard let recordsValue = dictionary["records"] else { throw FatCatIPCError.malformed("missing records") }
        ...
        case "message_added":
            guard let messageValue = dictionary["message"] else { throw FatCatIPCError.malformed("missing message") }
        ...
        case "session_list":
            guard let sessions = dictionary["sessions"] as? [[String: String]] else { throw FatCatIPCError.malformed("invalid session list") }
        ...
        case "plan":
            guard let steps = dictionary["steps"] as? [String] else { throw FatCatIPCError.malformed("invalid plan") }
            return .plan(requestID: try string("request_id"), sessionID: try string("session_id"), steps: steps)
        case "tool_call":
            guard let arguments = dictionary["arguments"] as? [String: String] else { throw FatCatIPCError.malformed("invalid tool call") }
        ...
        case "provider_inventory_result":
            guard let providers = dictionary["providers"] as? [[String: String]] else { throw FatCatIPCError.malformed("invalid provider inventory") }
            return .providerInventoryResult(requestID: try string("request_id"), providers: providers)
        case "provider_models_result":
            guard let models = dictionary["models"] as? [String] else { throw FatCatIPCError.malformed("invalid provider models") }
        ...
        case "state":
            guard let state = FatCatAgentState(rawValue: try string("state")) else { throw FatCatIPCError.malformed("unknown state") }
        ...
        case "observation":
            guard let activeApp = dictionary["active_app"] as? String,
                  ...
                  let confidence = dictionary["confidence"] as? Double else { throw FatCatIPCError.malformed("invalid observation") }
        ...
        default: throw FatCatIPCError.unsupportedType(type)
        }
    }

    private static func object(for message: FatCatIPCMessage) throws -> [String: Any] {
        switch message {
    ...
    private static func rejectCredentials(in value: [String: Any], path: String = "") throws {
        ...
        for (key, child) in value {
            if forbidden.contains(key.lowercased()) { throw FatCatIPCError.credentialField(path + key) }
    ...
    private static func decodeValue<T: Decodable>(_ value: Any, field: String) throws -> T {
        do {
            ...
            throw FatCatIPCError.malformed("invalid \(field)")
        }
    }
