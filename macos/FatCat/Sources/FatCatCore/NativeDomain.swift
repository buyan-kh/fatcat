...
public enum FatCatState: String, CaseIterable, Equatable, Sendable {
    case idle
    case listening
    case understanding
    case planning
...
public struct FatCatStateTransition: Equatable, Sendable {
    public let accepted: Bool
    public let state: FatCatState
    public let from: FatCatState
    public let to: FatCatState
    public let reason: String

    public init(accepted: Bool, state: FatCatState, from: FatCatState, to: FatCatState, reason: String) {
...
public struct FatCatStateMachine: Equatable, Sendable {
    public private(set) var state: FatCatState

    public init(state: FatCatState = .idle) {
        self.state = state
    }

    @discardableResult
    public mutating func transition(to next: FatCatState, verified: Bool = false, reason: String = "") -> FatCatStateTransition {
        let accepted = Self.canTransition(from: state, to: next, verified: verified)
        let previous = state
        if accepted { state = next }
        return FatCatStateTransition(
            accepted: accepted,
            state: state,
            from: previous,
            to: next,
    ...
    private static func canTransition(from: FatCatState, to: FatCatState, verified: Bool) -> Bool {
...
public final class PetPositionStore: @unchecked Sendable {
    ...
    private let key = "fatcat.pet.position"
