...
struct FatCatIPCTests {
    @Test func roundTripsSharedClientAndConversationEvents() throws {
        ...
        for message in messages {
            #expect(try FatCatIPCCodec.decodeLine(FatCatIPCCodec.encode(message: message)) == message)
        }
    }
    ...
    @Test func providerSetupMessagesRejectRawCredentialFields() throws {
        ...
        #expect(throws: FatCatIPCError.credentialField("api_key")) {
            try FatCatIPCCodec.decodeLine(raw)
        }
    }

    @Test func encodesTypedMessagesAsOneNewlineDelimitedRecord() throws {
        let message = FatCatIPCMessage.userMessage(
            requestID: "request-1",
        ...
        let encoded = try FatCatIPCCodec.encode(message: message)
        let decoded = try FatCatIPCCodec.decodeLine(encoded)
    ...
    @Test func decodesStreamingAssistantAndStateEvents() throws {
        let assistant = try FatCatIPCCodec.decodeLine(#"{"version":1,"type":"assistant_delta","request_id":"r1","session_id":"s1","text":"Hello"}"# + "\n")
        let state = try FatCatIPCCodec.decodeLine(#"{"version":1,"type":"state","state":"thinking"}"# + "\n")
    ...
    @Test func decodesSessionScopedStateEvents() throws {
        let state = try FatCatIPCCodec.decodeLine(#"{"version":1,"type":"state","state":"completed","session_id":"s1","request_id":"r1"}"# + "\n")

        #expect(state == .sessionState(.completed, sessionID: "s1", requestID: "r1"))
    }

    @Test func encodesObservationNullsAsValidJSON() throws {
        let data = try FatCatIPCCodec.encode(message: .observation(requestID: nil, activeApp: "Xcode", window: nil, visibleText: [], confidence: 0.8))
        #expect(try FatCatIPCCodec.decodeLine(data) == .observation(requestID: nil, activeApp: "Xcode", window: nil, visibleText: [], confidence: 0.8))
    }

    @Test func rejectsMalformedOrCredentialBearingMessages() throws {
        #expect(throws: FatCatIPCError.self) {
            try FatCatIPCCodec.decodeLine(Data("not json\n".utf8))
        }
        #expect(throws: FatCatIPCError.self) {
        try FatCatIPCCodec.decodeLine(Data((#"{"version":1,"type":"assistant_delta","api_key":"secret"}"# + "\n").utf8))
        }
    }
    ...
    @Test func roundTripsGenericHermesEvents() throws {
        ...
        #expect(try FatCatIPCCodec.decodeLine(FatCatIPCCodec.encode(message: .hermesEvent(event))) == .hermesEvent(event))
    }
    ...
    @Test func acceptsStructuredHermesEventDetails() throws {
        ...
        let decoded = try FatCatIPCCodec.decodeLine(Data((line + "\n").utf8))
