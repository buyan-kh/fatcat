  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(['system', 'light', 'dark'] as const).map((option) => (
              <Button key={option} variant={appearance === option ? 'default' : 'outline'} className={cn('capitalize', appearance === option && 'pointer-events-none')} onClick={() => onAppearanceChange(option)}>{option}</Button>
            ))}
          </div>
        </section>
        <Separator />
        <section className="space-y-2">
          <h3 className="text-sm font-medium">Hermes</h3>
          {providers.length > 0 ? providers.map((provider) => (
            <div key={provider.providerId} className="rounded-lg border p-3 text-xs">
        ...
        {diagnostics && (
          <><Separator /><section className="space-y-1 text-xs text-muted-foreground"><h3 className="font-medium text-foreground">Diagnostics</h3><p className="truncate">Agent: {diagnostics.agentPath}</p><p>{diagnostics.running ? 'Running' : 'Stopped'} · {diagnostics.lines.length} diagnostic lines</p></section></>
        )}
      </DialogContent>
    </Dialog>
  )
