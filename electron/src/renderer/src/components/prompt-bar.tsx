...
export function PromptBar({ workspacePath, isGenerating, disabled, onSend, onStop, onChooseWorkspace }: PromptBarProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-5 pb-5">
      <div className="surface-card focus-within:ring-2 focus-within:ring-ring/35">
        <Textarea
          ...
          className="min-h-12 max-h-40 resize-none border-0 bg-transparent px-3.5 pt-3 text-sm leading-6 shadow-none focus-visible:ring-0 dark:bg-transparent"
        />
        <div className="flex items-center gap-2 px-2.5 pb-2.5">
          <Button variant="ghost" size="sm" className="nav-control min-w-0 max-w-[60%] justify-start gap-1 px-2 text-xs text-muted-foreground" aria-label="Choose workspace" title={workspacePath} onClick={onChooseWorkspace}>
